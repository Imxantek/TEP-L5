...
//modyfikacja
template <typename E>
class CResult<const char*, E> {
public:
    static CResult<std::string, E> cOk(const char* value) {
        return CResult<std::string, E>::cOk(std::string(value));
    }

    static CResult<std::string, E> cFail(E* error) {
        return CResult<std::string, E>::cFail(error);
    }
    
    static CResult<std::string, E> cFail(std::vector<E*>& vErrors) {
        return CResult<std::string, E>::cFail(vErrors);
    }
};